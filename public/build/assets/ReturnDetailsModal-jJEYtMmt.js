import{y as B,z as I,o as l,e as V,w as p,a as c,u as i,b as e,g as d,t as o,c as u,n as R,i as x,F as U,A as L,C as M}from"./app--kvDeszr.js";import{Y as O,h as k,G as Y,V as G,S as Q}from"./transition--DXCpfbV.js";const W={class:"fixed inset-0 overflow-y-auto"},H={class:"flex items-center justify-center min-h-full p-4 text-center"},J={class:"flex items-center justify-between mb-6"},K={key:0,class:"space-y-4"},X={class:"grid grid-cols-1 md:grid-cols-2 gap-4"},Z={class:"bg-white p-4 rounded-xl shadow-sm border border-gray-200"},tt={class:"space-y-2 text-sm"},et={class:"text-gray-800 font-medium"},st={class:"text-gray-800 font-medium"},ot={class:"bg-white p-4 rounded-xl shadow-sm border border-gray-200"},at={class:"space-y-2 text-sm"},nt={class:"text-gray-800 font-medium"},rt={key:0},it={class:"text-gray-800 font-medium"},lt={class:"text-gray-800 font-medium"},dt={class:"text-gray-800 font-medium"},ut={class:"bg-white p-4 rounded-xl shadow-sm border border-gray-200"},mt={class:"overflow-x-auto"},pt={class:"w-full text-sm text-gray-700"},ct={class:"px-3 py-3"},xt={class:"font-medium text-gray-800"},gt={class:"text-gray-500 text-xs"},yt={class:"px-3 py-3 text-center text-gray-700"},bt={class:"px-3 py-3 text-center text-gray-700"},ft={class:"px-3 py-3 text-center font-semibold text-gray-800"},wt={class:"bg-blue-50 border-t-2 border-blue-600"},vt={class:"px-3 py-3 text-center font-bold text-green-600"},ht={key:0,class:"flex gap-3"},$t={class:"mt-6 flex justify-end gap-3"},Dt={key:1,class:"mt-4"},_t={class:"bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center justify-between"},Rt={class:"text-right"},kt={class:"text-sm text-gray-600"},St={__name:"ReturnDetailsModal",props:{open:Boolean,returnData:Object},emits:["close","update-status"],setup(s,{emit:C}){const m=M(),b=s,f=C;B(()=>b.open,a=>{a?document.body.style.overflow="hidden":document.body.style.overflow=""}),I(()=>{document.body.style.overflow=""});const w=()=>{f("close")},v=a=>{f("update-status",b.returnData,a),w()},T=a=>{const t=m.props.billSetting||{},n=["58mm","80mm","112mm","210mm"],h=(t.print_size||"80mm").toString(),$=n.includes(h)?h:"80mm",D=a.return_no||`RET-${a.id}`,S=a.return_date_formatted||a.return_date||"",z=a.customer_name||"Walk-in Customer",F=a.sale_no||"N/A",_=(a.products||[]).map(r=>({name:r.product_name,qty:r.quantity||0,price:parseFloat(r.price||0),total:parseFloat(r.total||0)})),P=parseFloat(a.returned_total||a.total_refund||_.reduce((r,A)=>r+A.total,0)),j=parseFloat(a.replacement_total||0),N=a.payment_due_label||"Settled",E=parseFloat(a.payment_due_amount||0).toFixed(2),g=m.props.currency||"",q=`
    <!DOCTYPE html>
    <html>
    <head>
      <meta charset="UTF-8">
    ...
    <body>
      <div class="receipt-container">
        ...
        <div class="totals">
          <div class="total-row"><span>Returned Total:</span><span>${g} ${P.toFixed(2)}</span></div>
          <div class="total-row"><span>Replacement Total:</span><span>${g} ${j.toFixed(2)}</span></div>
          <div class="total-row grand"><span>${N.toUpperCase()}:</span><span>${g} ${E}</span></div>
        </div>
